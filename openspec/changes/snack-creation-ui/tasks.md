...
- [ ] 5.1 **TEST**: Write `apps/web/src/modules/plates/components/__tests__/SnackVisualization.test.ts` — test renders 3 zones (HEALTHY_FAT, CEREAL_TUBER, FRUIT), displays food name per zone, empty zone shows empty-state, NO A/L balance indicator anywhere. Mount with mock `draftItems` fixture. Run `pnpm --filter web test:run` → RED. Covers REQ-SC7.
- [ ] 5.2 **COMPONENT**: Create `apps/web/src/modules/plates/components/SnackVisualization.vue` — accept `items: PlateItemDraft[]`, render 3-zone circular layout (HEALTHY_FAT left, CEREAL_TUBER top, FRUIT right), show food.name or empty placeholder per zone, NO balance bar, NO A/L indicator. Styled for 3 groups (not 4/5 like PlateVisualization). → GREEN. Satisfies REQ-SC7.
- [ ] 5.3 **REFACTOR**: Extract any duplicated zone-rendering patterns from PlateVisualization if applicable (unlikely — 3 zones vs 4/5 zones). Run `pnpm --filter web test:run` → GREEN.

### Phase 6: SnackBuilderDrawer Component (RED → GREEN → REFACTOR)

- [ ] 6.1 **TEST**: Write `apps/web/src/modules/plates/components/__tests__/SnackBuilderDrawer.test.ts` — test opens/closes drawer, displays 3 zones with suggested emphasis (11m baby → HEALTHY_FAT + CEREAL_TUBER emphasized, FRUIT not), name input required for save, save disabled when name empty, save button disabled when `!canSave` (tier limit), on-save warnings banner displays after successful save (mock `useSnackBuilder.saveSnack` returns `{snack, warnings: ['Missing fruit group']}`), Guardar triggers `saveSnack`, Limpiar triggers `clear`, hosts FoodSearchModal per group. Mount with mock baby age 11m. Run `pnpm --filter web test:run` → RED. Covers REQ-SC3, REQ-SC4, REQ-SC6.
- [ ] 6.2 **COMPONENT**: Create `apps/web/src/modules/plates/components/SnackBuilderDrawer.vue` — accept `visible`, `babyAge` props, emit `close`, use `useSnackBuilder({ ageMonths: () => babyAge, onSaved: (snack) => emit('saved', snack) })`, render right drawer (mirror PlateBuilderDrawer layout), name input (v-model `draftName`), optional stage selector (v-model `draftStageFor`), 3 build zones (HEALTHY_FAT, CEREAL_TUBER, FRUIT) with visual emphasis for `isSuggested(group)` (e.g. highlighted border or "Sugerido" badge), `<SnackVisualization :items="draftItems" />`, warnings banner (v-if `lastWarnings.length > 0` after save, dismissible), save button (:disabled="`!isValid || saving || !canSave`", @click="`handleSave`"), clear button (@click="`clear`"), hosts `<FoodSearchModal>` per group. `handleSave` async: calls `saveSnack()`, shows warnings banner if any, emits 'saved', closes drawer. → GREEN. Satisfies REQ-SC3, REQ-SC4, REQ-SC6.
- [ ] 6.3 **REFACTOR**: Extract any duplicated drawer layout/styles from PlateBuilderDrawer if significant overlap. Run `pnpm --filter web test:run` → all GREEN.

### Phase 7: Hygiene + PR-2 Verification

- [ ] 7.1 **TYPECHECK**: Typecheck web: `pnpm --filter web typecheck` → no errors.
- [ ] 7.2 **ALL TESTS GREEN**: Run `pnpm --filter web test:run` → all suites green (shared/api unchanged in PR-2).
- [ ] 7.3 **GIT STAGING**: Stage with explicit paths: `git add apps/web/src/modules/plates/components/ apps/web/src/shared/composables/` (avoids `.atl/skill-registry.md`).
