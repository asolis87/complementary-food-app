# Spec: Snack Creation UI (MODIFIED Capability)

> **Why**: menu-snacks shipped backend CRUD + menu assignment, but the catalog is unfillable from UI (empty-state dead-end). This delta adds CREATE + DELETE UI so users can build reusable snacks that appear in the menu picker.
>
> **Scope**: Frontend-only. No backend/schema changes. MVP = CREATE + DELETE only; NO EDIT.
>
> **Source**: `proposal.md` §In Scope + LOCKED decisions.

---

## Capability: Snack Management (MODIFIED)

Delta adds the creation + deletion UI surface to the headless snack entity that menu-snacks shipped. A dedicated "Colaciones" tab inside PlateListPage displays the user's saved snacks, with a SnackBuilderDrawer for manual per-group build (3 groups: HEALTHY_FAT, CEREAL_TUBER, FRUIT), age-suggested composition with non-blocking warnings (REQ-SM4), and NO A/L balance scoring.
...
_None. This change adds new UI surface to the existing snack-management capability without modifying any existing requirements._

---

## REMOVED Requirements

_None. This is an additive change._
...
- **Editing snacks**: No `updateSnack` action, no edit mode in SnackBuilderDrawer, no backend `PUT /api/snacks/:id` route. User must delete + recreate to fix mistakes.
- **Backend changes**: No new/modified API routes, services, schemas, or migrations. This change is frontend-only.
- **A/L balance / nutrition scoring for snacks**: No balance calculation, no balance bar, no score display (REQ-SM5 already specifies this).
- **Composition enforcement**: Warnings are informative only and never block save (REQ-SM4 already specifies non-blocking behavior).
- **Auto-composition**: System does NOT auto-fill snack items from age suggestions.
- **Snack duplication or filtering by food**: Not included in MVP.
